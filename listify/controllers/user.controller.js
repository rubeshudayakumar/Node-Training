...
const userLogin = async (req,res) => {
    ...
    try{
        if(!validator.userIdAndPasswordValidator(req.body)){
            response = httpWarnObject(req,constants.USERNAME_PASSWORD_INVALID);
        ...
        if(userIndex==-1){
            response =  httpWarnObject(req,constants.INCOREECT_USERNAME);
            return sendResponse(res,response);
        }
        if(!await bcrypt.compare(req.body.password,userData[userIndex].password)){
            response =  httpWarnObject(req,constants.INCORRECT_PASSWORD);
            return sendResponse(res,response);
        }
...
const userRegister = async (req,res) => {
    ...
    try{
        ...
        const userIndex = userService.isUserExists(userData,req.body.userName);
        if(userIndex!=-1){
            response = httpWarnObject(req,constants.USER_ALREADY_EXISTS);
            return sendResponse(res,response);
        }
